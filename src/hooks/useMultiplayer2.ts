...
export interface GameRoom2 {
  ...
  winner?: string;
  lastMove?: { subBoard: number; cell: number };
...
export const useMultiplayer2 = () => {
  const { user, userProfile } = useAuth();
  const [rooms, setRooms] = useState<GameRoom2[]>([]);
  const [currentRoom, setCurrentRoom] = useState<GameRoom2 | null>(null);
  const [players, setPlayers] = useState<Player[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  
  // Usar refs para evitar loops infinitos
  const currentRoomRef = useRef<GameRoom2 | null>(null);
  const playersRef = useRef<Player[]>([]);

  // Função utilitária para limpar objetos (remover undefined e null)
  const cleanObjectForFirebase = useCallback((obj: any) => {
    const cleanObj: any = {};
    
    Object.entries(obj).forEach(([key, value]) => {
      if (value === undefined || value === null) {
        // Pular valores undefined e null
        return;
      }
      ...
      if (Array.isArray(value)) {
        ...
        cleanObj[key] = value.map(item => {
          if (typeof item === 'object' && item !== null) {
            return cleanObjectForFirebase(item);
          }
          return item;
        });
      } else if (typeof value === 'object' && value !== null) {
        // Limpar objetos aninhados recursivamente
        cleanObj[key] = cleanObjectForFirebase(value);
      } else {
        cleanObj[key] = value;
      }
    });
  ...
  const checkWinner = (board: (string | '')[]): string | null => {
    console.log("checkWinner chamado com board:", board);
  ...
  const isSubBoardComplete = (subBoard: (string | '')[]): boolean => {
    return checkWinner(subBoard) !== null || checkDraw(subBoard);
  };
  ...
  const createRoom = useCallback(async (roomName?: string): Promise<string> => {
    ...
    try {
      const roomRef = push(ref(database, 'ttt2Rooms'));
      const roomId = roomRef.key!;
      const hostUsername = userProfile?.username || user.email?.split('@')[0] || 'Jogador 1';

      const newRoom: GameRoom2 = {
        id: roomId,
        hostId: user.uid,
        hostUsername: hostUsername,
        roomName: roomName || undefined,
        mainBoard: Array(9).fill(''),
        subBoards: Array(9).fill(null).map(() => Array(9).fill('')),
        currentPlayer: 'X',
        gameStatus: 'waiting',
        createdAt: Date.now()
      };

      await set(ref(database, `ttt2Rooms/${roomId}`), newRoom);
  ...
  const joinRoom = useCallback(async (roomId: string): Promise<void> => {
    ...
    try {
      const roomRef = ref(database, `ttt2Rooms/${roomId}`);
      ...
      const room: GameRoom2 = roomSnapshot.val();
      ...
      await set(ref(database, `ttt2Rooms/${roomId}`), {
        ...room,
        guestId: user.uid,
  ...
  // Fazer uma jogada
  const makeMove = useCallback(async (moveIndex: number): Promise<void> => {
    console.log("makeMove chamado com moveIndex:", moveIndex);
    
    const room = currentRoomRef.current;
    if (!room || !user) {
      console.log("makeMove falhou: room ou user não encontrado", { room: !!room, user: !!user });
      return;
    }

    const player = playersRef.current.find(p => p.id === user.uid);
    if (!player) {
      console.log("makeMove falhou: player não encontrado");
      return;
    }
    ...
    if (!isPlayerTurn) {
      console.log("makeMove falhou: não é o turno do jogador", { 
    ...
    // Converter índice único para sub-tabuleiro e célula
    const subBoardIndex = Math.floor(moveIndex / 9);
    const cellIndex = moveIndex % 9;

    // Verificar se a jogada é válida
    if (room.subBoards[subBoardIndex][cellIndex] !== '') {
      console.log("makeMove falhou: célula já ocupada", { subBoardIndex, cellIndex, value: room.subBoards[subBoardIndex][cellIndex] });
      return; // Célula já ocupada
    }

    // Verificar se o sub-tabuleiro está completo
    if (isSubBoardComplete(room.subBoards[subBoardIndex])) {
      console.log("makeMove falhou: sub-tabuleiro completo", { subBoardIndex });
      return; // Sub-tabuleiro completo
    }

    // Verificar se há próximo tabuleiro definido
    if (room.nextSubBoard !== null && room.nextSubBoard !== undefined && subBoardIndex !== room.nextSubBoard) {
      console.log("makeMove falhou: deve jogar no tabuleiro especificado", { 
        subBoardIndex, 
        nextSubBoard: room.nextSubBoard 
      });
      return; // Deve jogar no tabuleiro especificado
    }

    console.log("makeMove: jogada válida, processando...", { subBoardIndex, cellIndex, playerSymbol: player.symbol });

    try {
      // Fazer a jogada
      const newSubBoards = [...room.subBoards];
      newSubBoards[subBoardIndex][cellIndex] = player.symbol;

      // Verificar vitória no sub-tabuleiro
      const subWinner = checkWinner(newSubBoards[subBoardIndex]);
      let newMainBoard = [...room.mainBoard];
      let newGameStatus = room.gameStatus;
      let newWinner = room.winner;
      let newNextSubBoard: number | null = cellIndex;
      ...
      if (subWinner) {
        ...
        if (mainWinner) {
          newGameStatus = 'won';
          newWinner = mainWinner;
        } else if (checkDraw(newMainBoard)) {
          newGameStatus = 'draw';
          newWinner = '';
        }
      } else if (checkDraw(newSubBoards[subBoardIndex])) {
        // Marcar sub-tabuleiro como empate (usar string vazia para indicar empate)
        newMainBoard[subBoardIndex] = '';
      }

      // Verificar se o próximo tabuleiro está completo
      if (isSubBoardComplete(newSubBoards[newNextSubBoard])) {
        newNextSubBoard = null; // Permitir escolher qualquer tabuleiro
      }

      // Alternar jogador
      const newCurrentPlayer = room.currentPlayer === 'X' ? 'O' : 'X';

      // Atualizar sala
      const updatedRoom: GameRoom2 = {
        ...room,
        ...
        winner: newWinner,
        lastMove: { subBoard: subBoardIndex, cell: cellIndex },
        nextSubBoard: newNextSubBoard
      };

      // Limpar dados antes de enviar para Firebase
      const cleanRoom = cleanObjectForFirebase(updatedRoom);
      console.log("Enviando para Firebase:", cleanRoom);

      await set(ref(database, `ttt2Rooms/${room.id}`), cleanRoom);
  ...
  const resetGame = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (room.gameStatus === 'won' && room.winner && room.winner !== '') {
      ...
      const roomRef = ref(database, `ttt2Rooms/${room.id}`);
  ...
  const leaveRoom = useCallback(async (): Promise<void> => {
    ...
    try {
      if (room.hostId === user.uid) {
        ...
        await remove(ref(database, `ttt2Rooms/${room.id}`));
      } else if (room.guestId === user.uid) {
        // Convidado saindo - remover guestId
        await set(ref(database, `ttt2Rooms/${room.id}/guestId`), '');
        await set(ref(database, `ttt2Rooms/${room.id}/guestUsername`), '');
        await set(ref(database, `ttt2Rooms/${room.id}/gameStatus`), 'waiting');
      }
  ...
  const listRooms = useCallback(async (): Promise<void> => {
    ...
    try {
      const roomsRef = ref(database, 'ttt2Rooms');
      const snapshot = await get(roomsRef);

      if (snapshot.exists()) {
        const roomsData: GameRoom2[] = [];
  ...
  useEffect(() => {
    ...
    const roomRef = ref(database, `ttt2Rooms/${currentRoom.id}`);
    const unsubscribe = onValue(roomRef, (snapshot) => {
      if (snapshot.exists()) {
        const updatedRoom: GameRoom2 = snapshot.val();
        ...
        if (updatedRoom.resetRequestedBy && updatedRoom.resetConfirmedBy) {
          ...
          if (confirmedCount === totalPlayers) {
            ...
            if (updatedRoom.gameStatus === 'won' && updatedRoom.winner && updatedRoom.winner !== '') {
              // Se houve vitória, o perdedor começa
            ...
            set(ref(database, `ttt2Rooms/${updatedRoom.id}`), cleanResetRoom);
      ...
      if (snapshot.exists()) {
        ...
        snapshot.forEach((childSnapshot) => {
          ...
          if (roomAge > oneDayInMs) {
            deletePromises.push(remove(ref(database, `ttt2Rooms/${room.id}`)));
          }
        });
